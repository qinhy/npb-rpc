"""Minimal typed sum RPC example using the ZeroMQ backend."""
...
from npb_rpc import RpcContext, ZmqRpcClient, ZmqRpcServer, portable_ipc, portable_tcp
    ...
    if transport == "ipc":
        if not zmq.has("ipc"):
            raise SystemExit(
                ...
                "Use --transport tcp or the NNG IPC example on native Windows."
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the minimal ZeroMQ sum example")
