...
def run_client(args: argparse.Namespace, discovery: FilesystemDiscovery) -> None:
    ...
    if args.open_camera:
        response = client.open(CameraOpenRequest(device=args.device or "", timeout_s=args.control_timeout))
        print("open:", f"ok={response.ok}", f"online={response.online}", f"device={response.device!r}",
              f"generation={response.generation}", f"error={response.error!r}")
        return

    if args.close_camera:
        response = client.close(CameraCloseRequest(timeout_s=args.control_timeout))
        print("close:", f"ok={response.ok}", f"online={response.online}", f"device={response.device!r}",
              f"generation={response.generation}", f"error={response.error!r}")
        return

    status = client.status(EmptyRequest())
    print("status:", f"requested_open={status.requested_open}", f"online={status.online}",
          f"device={status.device!r}", f"generation={status.generation}", f"restarts={status.restart_count}",
          f"published={status.frames_published}", f"last_frame_ns={status.last_frame_ns}", f"error={status.error!r}")

    if args.single:
        frame = client.get_frame(CameraFrameRequest(stream=args.stream, thumbnail=args.thumbnail))
        if not frame.ok:
        ...
        print(f"wrote {path}: {frame.jpeg.nbytes} bytes, seq={frame.sequence}, "
              f"online={frame.camera_online}, captured_ns={frame.captured_ns}")
        return

    frames = client.frames(CameraFrameSetRequest())
    print("frame-set:", f"ok={frames.ok}", f"online={frames.camera_online}",
          f"generation={frames.generation}", f"restarts={frames.restart_count}", f"error={frames.error!r}")
    ...
    outputs = (
        ...
        ("rgb_thumbnail.jpg", frames.rgb_thumbnail, frames.rgb_thumbnail_sequence, frames.rgb_thumbnail_captured_ns),
        ("left_thumbnail.jpg", frames.left_thumbnail, frames.left_thumbnail_sequence, frames.left_thumbnail_captured_ns),
        ("right_thumbnail.jpg", frames.right_thumbnail, frames.right_thumbnail_sequence, frames.right_thumbnail_captured_ns),
    )
...
def show_services(discovery: FilesystemDiscovery) -> None:
    ...
    for service in services:
        ...
        for instance in discovery.list_instances(service):
            print(f"  {instance.instance_id}  {instance.backend}  {instance.endpoint}  "
                  f"{instance.hostname} pid={instance.pid}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fault-tolerant discovered DepthAI camera RPC service")
    ...
    control.add_argument("--open-camera", action="store_true")
    control.add_argument("--close-camera", action="store_true")
...
def main() -> None:
    ...
    if args.role == "server":
        ...
        endpoint = args.endpoint or endpoint_for(args.backend, args.transport, server_name, args.host)
        print(f"serving {args.service!r} as {server_name!r} via {args.backend} at {endpoint}; registry: {discovery.root}")
        run_server(
            endpoint,
